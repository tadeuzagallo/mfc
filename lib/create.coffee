			...
			fs.mkdir "./#{name}/app/views", 0777, ->
				fs.mkdir "./#{name}/app/views", 0777, ->
					fs.mkdir "./#{name}/app/views/main", 0777, ->
						fs.writeFile "./#{name}/app/views/main/index.html", '<b>tz microframework for CoffeeScript</b>\n<ul>\n\t<li>application: ' + name + '</li>\n\t<li>url: <%= @url %></li>\n\t<li>controller: <%= @attr \'controller\' %></li>\n\t<li>action: <%= @attr \'action\'  %></li>\n\t<li>layout: <%= if ((layout = @attr \'layout\')) then "#{@attr \'basePath\'}/app/views/layouts/#{layout}" else \'null\' %></li>\n\t<li>view: <%= if (view = @attr \'view\').length then "#{@attr \'basePath\'}/app/views/#{@attr \'controller\'}/#{view}" else \'null\' %></li>\n\t<li>params:\n\t\t<ul>\n\t\t\t<% x = 0 %>\n\t\t\t<% for key, value of do @params: %>\n\t\t\t\t<% x++ %>\n\t\t\t\t<li><%= key %>: <%= value or \'empty\' %></li>\n\t\t\t<% end %>\n\t\t\t<% unless x: %>\n\t\t\t\t<li>empty</li>\n\t\t\t<% end %>\n\t\t</ul>\n\t</li>\n</ul>'
